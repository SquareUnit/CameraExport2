...
public class InputsManager : MonoBehaviour
{
    ...
    [HideInInspector] public bool crossPressed { get; private set; }
    ...
    #region Inputs Constants. Raw : Use at your own risk
    // Constants variables are naturally hidden in inspector even when they are public.
    ...
    public void SetInputsHash()
    {
        ...
        verticalHash = ReInput.mapping.GetActionId("vertical");
        ...
        crossPressedHash = ReInput.mapping.GetActionId("crossDown");
    }
    ...
    {
        leftStickRaw = new Vector3(player.GetAxisRaw(horizontalHash.GetHashCode()), 0.0f, player.GetAxisRaw(verticalHash.GetHashCode()));
    ...
    private void GetDebugMenuInputs()
    {
        ...
        crossPressed = player.GetButtonTimedPress(crossRightHash.GetHashCode(), 2f);
    ...
    private Vector3 CameraDeadZone(Vector3 rightStickRaw)
    {
        ...
        if (rightStickRaw.x >= 0)
        {
            ...
            {
                remapedVal = ((rightStickRaw.x - 0.25f) / 0.75f);
        ...
        if (rightStickRaw.y >= 0)
        {
            ...
            {
                remapedVal = ((rightStickRaw.y - 0.25f) / 0.75f);
